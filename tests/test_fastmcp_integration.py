...
def temp_storage():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("src.arxiv_mcp_server.config.Settings.STORAGE_PATH", Path(tmpdir)):
            yield Path(tmpdir)
    ...
    async with Client(mcp) as client:
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            if result_text.startswith("Error:"):
                # For error cases, just verify the error message
                assert "Error" in result_text
            else:
                # For success cases, parse JSON
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            content = json.loads(download_result[0].text)
            assert "status" in content
        
        # Test 5: List papers
        with patch("arxiv.Client") as mock_client_class:
            mock_client = mock_client_class.return_value
            mock_client.results = lambda *args, **kwargs: []  # Return empty list synchronously
            
            list_result = await client.call_tool("list_papers", {})
            content = json.loads(list_result[0].text)
        ...
        with patch("pathlib.Path.exists") as mock_exists, \
            ...
            content = json.loads(read_result[0].text)
        ...
        print('DEBUG prompt_result dir:', dir(prompt_result))
        # Try str(prompt_result) or .content if available
        result_str = str(prompt_result)
        assert "2103.12345" in result_str
